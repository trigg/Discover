...
class Settings(Gtk.Application):
    ...
    def __init__(self, a, b, config_file, rpc_file, channel_file, args):
        super().__init__(application_id=a, flags=b)
        self.args = args
    ...
    def start(self, _x):
        ...
        with importlib_resources.as_file(
            importlib_resources.files("discover_overlay") / "glade/settings.xml"
        ) as ui_path:
            builder.add_from_file(ui_path.as_posix())
        ...
        self.widget["overview_main_text"].set_markup(
            ...
            % (
                ...
                "</span>\n\n",
                _(
                    "Discover-Overlay is a GTK4 overlay written in Python3."
        ...
        if self.spinning_focus:
            match event.keyval:
                ...
                case Gdk.KEY_Down:
                    ...
                    self.spinning_focus.set_value(value - step)
                case Gdk.KEY_space:
                    self.spinning_focus.get_style_context().remove_provider(
                        self.super_focus
                    )
                    self.spinning_focus = None
                case Gdk.KEY_Escape:
            ...
            match event.keyval:
                ...
                case Gdk.KEY_Down:
                    ...
                    self.scale_focus.set_value(value - 0.1)
                case Gdk.KEY_space:
                    self.scale_focus.get_style_context().remove_provider(
                        self.super_focus
                    )
                    self.scale_focus = None
                case Gdk.KEY_Escape:
    ...
    def read_config(self):
        ...
        self.widget["voice_talking_foreground"].set_rgba(
            self.make_colour(
                config.get("main", "fg_hi_col", fallback="[1.0,1.0,1.0,1.0]")
            )
        )

        self.widget["voice_talking_background"].set_rgba(
            self.make_colour(config.get("main", "hi_col", fallback="[0.0,0.0,0.0,0.5]"))
        )

        self.widget["voice_talking_border"].set_rgba(
            self.make_colour(config.get("main", "tk_col", fallback="[0.0,0.7,0.0,0.2]"))
        )

        self.widget["voice_idle_foreground"].set_rgba(
            self.make_colour(config.get("main", "fg_col", fallback="[1.0,1.0,1.0,1.0]"))
        )

        self.widget["voice_idle_background"].set_rgba(
            self.make_colour(config.get("main", "bg_col", fallback="[0.0,0.0,0.0,0.5]"))
        )

        self.widget["voice_idle_border"].set_rgba(
            self.make_colour(config.get("main", "bo_col", fallback="[0.0,0.0,0.0,0.0]"))
        )

        self.widget["voice_mute_foreground"].set_rgba(
            self.make_colour(config.get("main", "mt_col", fallback="[0.6,0.0,0.0,1.0]"))
        )

        self.widget["voice_mute_background"].set_rgba(
            self.make_colour(
                config.get("main", "mt_bg_col", fallback="[0.0,0.0,0.0,0.5]")
            )
        )

        self.widget["voice_avatar_background"].set_rgba(
            self.make_colour(
                config.get("main", "avatar_bg_col", fallback="[0.0,0.0,0.0,0.0]")
            )
        )

        self.widget["voice_avatar_opacity"].set_value(
            config.getfloat("main", "icon_transparency", fallback=1.0)
        )
        ...
        self.widget["voice_show_title"].set_active(
            config.getboolean("main", "show_title", fallback=False)
        )

        show_avatar = config.getboolean("main", "show_avatar", fallback=True)
        self.widget["voice_show_avatar"].set_active(show_avatar)
        self.voice_show_avatar_hide_others(show_avatar)

        self.widget["voice_show_connection_status"].set_active(
            config.getboolean("main", "show_connection", fallback=False)
        )
        ...
        self.widget["voice_text_side"].set_active(
            config.getint("main", "text_side", fallback=3)
        )
        ...
        self.widget["text_colour"].set_rgba(
            self.make_colour(config.get("text", "fg_col", fallback="[1.0,1.0,1.0,1.0]"))
        )

        self.widget["text_background_colour"].set_rgba(
            self.make_colour(config.get("text", "bg_col", fallback="[0.0,0.0,0.0,0.5]"))
        )
        ...
        self.widget["text_show_attachments"].set_active(
            config.getboolean("text", "show_attach", fallback=True)
        )
        ...
        self.widget["notification_text_colour"].set_rgba(
            self.make_colour(
                config.get("notification", "fg_col", fallback="[1.0,1.0,1.0,1.0]")
            )
        )
        self.widget["notification_background_colour"].set_rgba(
            self.make_colour(
                config.get("notification", "bg_col", fallback="[0.0,0.0,0.0,0.5]")
            )
        )
        ...
        self.widget["notification_icon_padding"].set_value(
            config.getint("notification", "icon_padding", fallback=8)
        )

        self.widget["notification_icon_size"].set_value(
            config.getint("notification", "icon_size", fallback=32)
        )

        self.widget["notification_padding_between"].set_value(
            config.getint("notification", "padding", fallback=8)
        )

        self.widget["notification_border_radius"].set_value(
    ...
    def update_toggle_overlay(self, _a=None, _b=None):
        ...
        self.widget["core_hide_overlay"].handler_block(self.hidden_overlay_handler)
    ...
    def voice_align_1_changed(self, button):
        ...
        if button.get_active() == 0:
            value = "left"
        elif button.get_active() == 1:
            value = "middle"
        else:
            value = "right"
        self.config_set("main", "x_align", f"{value}")
    ...
    def voice_text_vertical_offset_changed(self, button):
        self.config_set("main", "text_baseline_adj", f"{int(button.get_value())}")

    def voice_vertical_padding_changed(self, button):
        self.config_set("main", "vert_edge_padding", f"{int(button.get_value())}")

    def voice_horizontal_padding_changed(self, button):
        self.config_set("main", "horz_edge_padding", f"{int(button.get_value())}")
    ...
    def voice_display_speakers_grace_period(self, button):
        self.config_set("main", "only_speaking_grace", f"{int(button.get_value())}")
    ...
    def voice_avatar_opacity_changed(self, button):
        self.config_set("main", "icon_transparency", f"{button.get_value():.2f}")
    ...
    def text_align_1_changed(self, button):
        ...
        if button.get_active() == 0:
            ...
            value = "right"
        self.config_set("text", "x_align", f"{value}")

    def text_align_2_changed(self, button):
        value = "none"
        if button.get_active() == 0:
            value = "top"
        elif button.get_active() == 1:
            ...
            value = "bottom"
        self.config_set("text", "y_align", f"{value}")
    ...
    def notification_align_1_changed(self, button):
        ...
        if button.get_active() == 0:
            ...
            value = "right"
        self.config_set("notification", "x_align", f"{value}")
    ...
    def notification_align_2_changed(self, button):
        ...
        if button.get_active() == 0:
            ...
            value = "bottom"
        self.config_set("notification", "y_align", f"{value}")
    ...
    def notification_icon_padding_changed(self, button):
        self.config_set("notification", "icon_padding", f"{int(button.get_value())}")

    def notification_icon_size_changed(self, button):
        self.config_set("notification", "icon_size", f"{int(button.get_value())}")

    def notification_padding_between_changed(self, button):
        self.config_set("notification", "padding", f"{int(button.get_value())}")
    ...
    def inactive_fade_opacity_changed(self, button):
        self.config_set("main", "fade_out_limit", f"{button.get_value():2.2f}")
    ...
    def inactive_fade_time_changed(self, button):
        self.config_set("main", "inactive_fade_time", f"{int(button.get_value())}")
